...
Run only focused checks needed for this TODO. Do not run the final project validator or broad end-to-end validation unless this TODO explicitly requires it. After a plausible fix, run the smallest focused verification that can prove the current acceptance criteria; if it passes, stop immediately instead of continuing exploration.
