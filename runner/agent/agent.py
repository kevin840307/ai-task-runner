...
class Agent:
    ...
    def _enrich_loop_diagnostics(
        ...
        *,
        ...
        if not allow_context_recovery:
            ...
            diagnostics["context_compress_reason"] = "session_reset"
            return
    ...
    def _prepare_session_recovery(self, error: BackendError, message: str) -> None:
        diagnostics = error.diagnostics
        loop_detected = bool(diagnostics.get("loop_type")) or should_reset_session(message)
        ...
        if not self.session_id:
            self._recoverable_session_failures = 0
            diagnostics["session_recovery_action"] = "retry_fresh_session"
            return

        failures = getattr(self, "_recoverable_session_failures", 0) + 1
        reset_session = failures >= SESSION_RECOVERABLE_FAILURES_BEFORE_RESET
        self._enrich_loop_diagnostics(
            error,
            allow_context_recovery=not reset_session,
        )
        if reset_session:
            diagnostics["session_recovery_action"] = "reset_session"
            self.session_id = ""
            self._recoverable_session_failures = 0
            return
    ...
    def _raise_backend_error(
        ...
        if self.session_id and is_session_invalid_error(message):
            ...
            self._prepare_session_recovery(error, message)
        self._finish_debug(
            call_session_id,
            prompt,
    ...
    def prepare_project(self) -> list[Path]:
        ...
        try:
            from ..runtime.extensions import register_resources
...
def agent_arguments(config, mode, *, allow_project_read=False):
...
def create_agent(config, root, debug_dir=None, *, mode="runtime", session_id="", timeout=None, allow_project_read=False, extra_args=None, constructor=Agent):
    """Create one agent directly; no factory lifecycle is required."""
    ...
    args = (
        ...
        else agent_arguments(config, mode, allow_project_read=allow_project_read)
    )
    return constructor(
...
def configure_agent(agent, config, mode, *, allow_project_read=False):
    """Switch stage capabilities on an existing live agent/session."""
    agent.set_extra_args(agent_arguments(config, mode, allow_project_read=allow_project_read))


class AgentClient(Agent):
    """Backward-compatible public name; new code should use Agent."""
