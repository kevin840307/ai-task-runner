...
from ...model.prompt import always_instructions, model_rules
...
class StageResult:
    """Facts from one Stage plus optional work to enqueue next."""
...
class StageContext:
    ...
    def save_session(self) -> None:
        self.state.model_session_id = self.model.session_id
        self.save_state()

    def reset_sessions(self) -> None:
        """Drop every cached model session so the next call starts fresh."""
        for value in (self.model, *self.scratch.values()):
            if hasattr(value, "session_id"):
                value.session_id = ""
        self.state.model_session_id = ""
        self.save_state()

    def template_values(self, stage: str, previous: StageResult | None = None) -> dict[str, Any]:
        return {
            "stage": stage,
            ...
            "rules": model_rules(self.root),
            "always_instructions": always_instructions(self.root),
        }
