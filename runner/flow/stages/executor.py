...
from ...utils.project import changed_project_files, project_manifest
from .base import Stage, StageContext, StageExecution, StageResult
...
class StageExecutor:
    ...
    def run(self, stage: Stage, ctx: StageContext, previous: StageResult | None = None) -> StageResult:
        ...
        if retry_attr:
            configured_retry = getattr(ctx.args, retry_attr)
        same_retry_limit = max(0, int(
            ctx.args.task_recovery_threshold if configured_retry is None else configured_retry
        ))
        attempt = 0
        retry_mode = "initial"
        previous_error = ""
    ...
    def _attempt(self, stage: Stage, ctx: StageContext, previous: StageResult | None) -> StageResult:
        ...
        if violations:
            ...
            result = (
                replace(result, changed_files=[], feedback=[*result.feedback, *messages])
                if result.status == "error"
                else StageResult.error_result(stage.name, RunnerError("; ".join(messages)))
            )
        return result
