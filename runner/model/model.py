"""Public model client API and session/recovery coordination."""
from __future__ import annotations

import time
from collections.abc import Callable, Sequence
from pathlib import Path
from typing import NoReturn, Protocol, TypeVar

from ..config.defaults import DEFAULT_LOOP_CONTEXT_COMPRESS_THRESHOLD
from ..errors import RunnerError, diagnostic_detail
from ..runtime import progress as runner_status
from .backend import BackendResult, ModelBackend, ModelMode, ensure_project_rules, split_command
from .errors import BackendError, ModelError
from .session import is_session_invalid_error, is_transient_service_error
from .diagnostics import error_result, prepare_session_recovery

T = TypeVar("T")

class Model(Protocol):
    session_id: str
    root: Path
    extra_args: Sequence[str]

    def ask(self, prompt: str, idle_timeout_after_change: float = 0, change_detected: Callable[[], bool] | None = None, timeout: int | None = None) -> str: ...
    def set_extra_args(self, extra_args: Sequence[str]) -> None: ...


class ModelClient:
    def __init__(
        self,
        backend: str,
        command: str | None,
        root: Path,
        extra_args: Sequence[str],
        session_id: str = "",
        timeout: int = 7200,
        debug_dir: Path | None = None,
        loop_context_compress: bool = False,
        loop_context_compress_threshold: float = DEFAULT_LOOP_CONTEXT_COMPRESS_THRESHOLD,
    ) -> None:
        try:
            from ..backends import create_backend
            self._backend = create_backend(
                backend, command, root, extra_args, timeout
            )
        except BackendError as error:
            raise ModelError(str(error), transient=is_transient_service_error(str(error))) from error

        # Preserve the simple public attributes from the original single-file ModelClient.
        self.backend = self._backend.name
        self.base_command = self._backend.base_command
        self.root = self._backend.root
        self.extra_args = self._backend.extra_args
        self.session_id = session_id
        self.timeout = timeout
        self.debug_dir = debug_dir
        self.loop_context_compress = loop_context_compress
        self.loop_context_compress_threshold = loop_context_compress_threshold

    def invoke(
        self,
        call,
        status: str,
        detail: str,
        retry_wait: float,
        retry_max_wait: float,
        *,
        max_elapsed: float = 0,
    ):
        """Run one model operation with transport-only reliability.

        Stage retry/session replacement is owned by StageExecutor; stages return final follow-up facts to Pipeline.
        """
        return _call_with_backoff(
            call, status, detail, retry_wait, retry_max_wait, max_elapsed=max_elapsed
        )

    @property
    def name(self) -> str:
        return self.backend

    def set_extra_args(self, extra_args: Sequence[str]) -> None:
        """Switch stage capabilities without replacing this client or session."""
        values = list(extra_args)
        self.extra_args = values
        self._backend.extra_args = values

    def _publish_model_event(
        self, kind: str, session_id: str, text: str, call_id: str = "", error: str = "", **metadata
    ) -> str:
        from datetime import datetime, timezone
        value = call_id or datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S.%fZ")
        try:
            from ..bootstrap import current_runtime
            current_runtime().events.publish({
                "type": kind,
                "debug_dir": str(self.debug_dir) if self.debug_dir else "",
                "call_id": value,
                "backend": self.backend,
                "cwd": str(self.root),
                "session": session_id,
                "text": text,
                "error": error,
                **metadata,
            })
        except RuntimeError:
            pass
        return value

    def _finish_debug(
        self, session_id: str, prompt: str, result: str, call_id: str, error: str = "", **metadata
    ) -> None:
        self._publish_model_event("model.result", session_id, result, call_id, error, **metadata)

    def _raise_backend_error(
        self,
        error: BackendError,
        prompt: str,
        call_session_id: str,
        debug_call_id: str,
    ) -> NoReturn:
        if error.session_id and not self.session_id:
            self.session_id = error.session_id

        message = str(error)
        expired_session = ""
        if self.session_id and is_session_invalid_error(message):
            expired_session = self.session_id
            self.session_id = ""
            error.diagnostics["session_recovery_action"] = "reset_session"
        else:
            prepare_session_recovery(self, error, message)
        actual_session_id = error.session_id or self.session_id or call_session_id
        self._finish_debug(
            actual_session_id,
            prompt,
            error_result(self._backend, error),
            debug_call_id,
            diagnostic_detail(error),
            session_mode="resume" if call_session_id else "new",
            previous_session=call_session_id,
        )
        if expired_session:
            raise ModelError(
                f"session {expired_session} is unavailable; "
                "a new session will continue from runner state"
            ) from error
        raise ModelError(
            message,
            transient=is_transient_service_error(message),
        ) from error

    def ask(
        self,
        prompt: str,
        idle_timeout_after_change: float = 0,
        change_detected: Callable[[], bool] | None = None,
        timeout: int | None = None,
    ) -> str:
        previous_timeout = self.timeout
        previous_backend_timeout = self._backend.timeout
        if timeout is not None:
            self.timeout = timeout
            self._backend.timeout = timeout
        call_session_id = self.session_id
        session_mode = "resume" if call_session_id else "new"
        debug_call_id = self._publish_model_event(
            "model.prompt", call_session_id, prompt, session_mode=session_mode
        )
        try:
            result = self._backend.ask(
                prompt,
                self.session_id,
                idle_timeout_after_change,
                change_detected,
            )
        except BackendError as error:
            self._raise_backend_error(
                error,
                prompt,
                call_session_id,
                debug_call_id,
            )
        finally:
            self.timeout = previous_timeout
            self._backend.timeout = previous_backend_timeout
        if result.session_id and not self.session_id:
            self.session_id = result.session_id
        actual_session_id = self.session_id or result.session_id or call_session_id
        self._finish_debug(
            actual_session_id, prompt, result.text, debug_call_id,
            session_mode=session_mode, previous_session=call_session_id,
        )
        return result.text

    def prepare_project(self) -> list[Path]:
        paths = self._backend.prepare_project()
        try:
            from ..bootstrap import register_resources
            register_resources(paths)
        except RuntimeError:
            pass
        return paths

    def update_goal_reference(self, goal_file: str | None) -> None:
        self._backend.update_goal_reference(goal_file)

    def _decode(self, raw: str) -> str:
        """Compatibility delegate for existing integrations/tests."""
        result = self._backend.decode(raw)
        if result.session_id and not self.session_id:
            self.session_id = result.session_id
        return result.text




def model_arguments(config, mode, *, allow_project_read=False):
    """Build backend arguments for one stage without a factory object."""
    from ..backends import configure_model_args
    return configure_model_args(
        config.backend, mode, getattr(config, "agent_arg", []),
        allow_project_read=allow_project_read,
        sandbox=getattr(config, "sandbox", False),
    )


def create_model(config, root, debug_dir=None, *, mode="runtime", session_id="", timeout=None, allow_project_read=False, extra_args=None, constructor=ModelClient):
    """Create one model client directly; no factory lifecycle is required."""
    from ..backends import configure_sandbox_args
    from ..config.defaults import DEFAULT_AGENT_TIMEOUT, DEFAULT_LOOP_CONTEXT_COMPRESS_THRESHOLD
    args = (
        configure_sandbox_args(config.backend, extra_args, sandbox=getattr(config, "sandbox", False))
        if extra_args is not None
        else model_arguments(config, mode, allow_project_read=allow_project_read)
    )
    return constructor(
        backend=config.backend, command=config.command, root=root, extra_args=args,
        session_id=session_id,
        timeout=getattr(config, "agent_timeout", DEFAULT_AGENT_TIMEOUT) if timeout is None else timeout,
        debug_dir=debug_dir,
        loop_context_compress=getattr(config, "loop_context_compress", False),
        loop_context_compress_threshold=getattr(config, "loop_context_compress_threshold", DEFAULT_LOOP_CONTEXT_COMPRESS_THRESHOLD),
    )


def configure_model(model, config, mode, *, allow_project_read=False):
    """Switch stage capabilities on an existing live model session."""
    model.set_extra_args(model_arguments(config, mode, allow_project_read=allow_project_read))




def _call_with_backoff(
    action: Callable[[], T],
    status: str,
    detail: str,
    wait: float,
    max_wait: float,
    *,
    max_elapsed: float = 0,
) -> T:
    """Retry only transient API/service failures; StageExecutor owns every real failure."""
    delay = max(0.0, wait)
    started = time.monotonic()
    retrying = False
    while True:
        if retrying:
            runner_status.start(status, detail)
        try:
            return action()
        except RunnerError as error:
            if not bool(getattr(error, "transient", False)):
                raise
            elapsed = time.monotonic() - started
            if max_elapsed > 0 and elapsed >= max_elapsed:
                raise
            runner_status.stop("API/服務異常，等待後重試", diagnostic_detail(error))
            retrying = True
            if delay:
                sleep_for = min(delay, max_elapsed - elapsed) if max_elapsed > 0 else delay
                if sleep_for > 0:
                    time.sleep(sleep_for)
                delay = min(max_wait, max(wait, delay * 2))



__all__ = ["Model", "ModelBackend", "ModelMode", "BackendResult", "ModelClient", "ensure_project_rules", "split_command", "create_model", "configure_model", "model_arguments"]
