...
{{ planning.source_instruction }}

Inspection summary when available:
{{ planning.inspection_summary }}

Produce the best concrete {{ planning.mode }} plan possible from the evidence already available.
...
Return at least {{ planning.minimum_tasks }} ordered task(s), using only as many tasks as the goal's real deliverables require. Never split work to target a task count or manufacture preparation/read/check tasks.
