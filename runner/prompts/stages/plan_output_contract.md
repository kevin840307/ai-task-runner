...
For each TODO, choose the smallest ordered `steps` sequence needed from Available task Stages. Use only listed Stage names. When a review Stage is available, it must be the final step. Use optional Stages only when they add value to that TODO; do not add a Stage merely because it exists.
