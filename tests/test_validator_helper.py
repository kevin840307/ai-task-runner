...
from runner.safety.policy import protected_paths


def test_runner_validator_execution_has_no_helper_filename_policy():
    source = Path("runner/workflow/validation/file.py").read_text(encoding="utf-8")
    core = Path("runner/engine/core.py").read_text(encoding="utf-8")
