...
def test_supervisor_resumes_after_abnormal_worker_exit(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(supervisor_module, "cleanup_orphan", lambda current, pid: calls.append(("cleanup", pid)))
...
def test_cleanup_orphan_only_for_matching_worker(tmp_path, monkeypatch):
    ...
    supervisor_module.cleanup_orphan(request, 999)
    assert calls == []
    assert active.exists()

    supervisor_module.cleanup_orphan(request, 100)
    assert calls
