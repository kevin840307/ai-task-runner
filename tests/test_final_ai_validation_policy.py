...
def _ai_args(total=3, required=2):
    from types import SimpleNamespace
    return SimpleNamespace(
        backend="qwen", command=None, sandbox=False, agent_arg=[], agent_timeout=0,
        agent_idle_after_change_timeout=0, retry_wait=0, retry_max_wait=0,
        final_ai_validations=total, final_ai_required_passes=required,
        loop_context_compress=False, loop_context_compress_threshold=50,
    )
...
def test_ai_validation_stops_after_quorum_in_independent_sessions(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ai_validation, "create_agent", lambda *a, **k: sessions.append("") or object())
    monkeypatch.setattr(ai_validation, "readonly_structured_call", lambda *a, **k: next(replies))
    monkeypatch.setattr(ai_validation, "retry_model_call", lambda call, *a, **k: call())

    state = RunState(run_id="r", goal="g", project_root=str(tmp_path))
    passed, output = ai_validation.run_ai_validator(
        _ai_args(), tmp_path, tmp_path / ".work", state, 1
    )
...
def test_run_ai_validator_uses_majority_and_runs_all_sessions(monkeypatch, tmp_path):
    ...
    state = RunState(run_id="r", goal="g", project_root=str(tmp_path))
    passed, output = ai_validation.run_ai_validator(
        _ai_args(required=0), tmp_path, tmp_path / ".work", state, 1, "custom"
    )
    assert passed is True
    assert sessions == ["", "", ""]
...
def test_ai_validation_stops_when_quorum_is_impossible(monkeypatch, tmp_path):
    ...
    sessions = []
    result = {"passed": False, "reason": "issue", "missing_items": ["x"], "checks_run": [], "suggested_checks": []}
    monkeypatch.setattr(ai_validation, "create_agent", lambda *a, **k: sessions.append("") or object())
    monkeypatch.setattr(ai_validation, "readonly_structured_call", lambda *a, **k: result)
    monkeypatch.setattr(ai_validation, "retry_model_call", lambda call, *a, **k: call())
...
def _validate_stage(tmp_path, ai_prompt="check architecture"):
    from types import SimpleNamespace
    from runner.engine.models import RunState
    from runner.workflow.stages import StageContext, ValidateStage

    state = RunState(run_id="r", goal="g", project_root=str(tmp_path))
    args = SimpleNamespace(
        validator_timeout=10, validator_arg=[], ai_validator_prompt=ai_prompt,
        validator_prompt="", agent_timeout=0, agent_idle_after_change_timeout=0,
        retry_wait=0, retry_max_wait=0, final_ai_validations=1,
        final_ai_required_passes=0,
    )
    ctx = StageContext(
        args=args, root=tmp_path, work=tmp_path / ".work", state=state,
        agent=SimpleNamespace(session_id=""), state_file=tmp_path / ".work/state.json",
        validator=tmp_path / "validator.py", ai_validation=False,
        save_state=lambda: None, set_stage=lambda *a: None,
    )
    return ValidateStage(ctx)


def test_mixed_validation_runs_hard_gate_then_ai_vote(monkeypatch, tmp_path):
    import runner.workflow.stages as stages
    calls = []
    monkeypatch.setattr(stages, "run_file_validator", lambda *a, **k: (calls.append("file") or True, "hard ok"))
    monkeypatch.setattr(stages, "run_ai_validator", lambda *a, **k: (calls.append(("ai", a[-1])) or True, "ai ok"))
    outcome = _validate_stage(tmp_path).run()
    assert outcome.status == "pass"
    assert calls == ["file", ("ai", "check architecture")]
    assert outcome.output == "FILE_VALIDATION_PASS\nai ok"


def test_mixed_validation_skips_ai_when_hard_gate_fails(monkeypatch, tmp_path):
    import runner.workflow.stages as stages
    calls = []
    monkeypatch.setattr(stages, "run_file_validator", lambda *a, **k: (calls.append("file") or False, "hard fail"))
    monkeypatch.setattr(stages, "run_ai_validator", lambda *a, **k: (_ for _ in ()).throw(AssertionError("AI must not run")))
    outcome = _validate_stage(tmp_path).run()
    assert outcome.status == "fail"
    assert outcome.output == "hard fail"
    assert calls == ["file"]
