...
The tool reuses the production workflow loader, Pipeline, StageResult, and Stage finish/result handlers. Only Stage execution results are mocked.
...
2. `dryrunexample/workflow.yaml`, a custom workflow where `check` fails three times, exercises `recover` and `max_results`, then still reaches final completion.
