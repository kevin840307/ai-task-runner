...
def validate(root: Path)->None:
    ...
    report_path=root/'report.json'; assert report_path.is_file(),'missing report.json'; actual=parse_json(report_path.read_text(encoding='utf-8'),'report.json'); assert actual==EXPECTED_REPORT,'unexpected report.json:\n'+json.dumps(actual,indent=2,sort_keys=True)
