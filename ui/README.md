...
9. Workflow/Prompt assets expose a compact asset menu with **Rename** and **Duplicate**. Rename stays in the same folder/scope and is blocked for referenced Prompts. Duplicate opens a compact target form so Workflow and Prompt copies can be placed in a selected Custom subfolder or existing Project Workflow folder; System assets remain immutable and duplicate into Custom. Prompt Import uses the same explicit folder selection instead of guessing the target folder.
