(() => {
  ...
  function filterItems(items, query) { const needle = normalizeQuery(query); if (!needle) return [...(items || [])]; return (items || []).filter((item) => [item.name, item.display_name, item.path, item.scope, item.group].some((value) => String(value || "").toLocaleLowerCase().includes(needle))); }
