...
def validate(root: Path) -> None:
    ...
    assert lines and lines[0]=='# Incident Report', f"Invalid H1: expected '# Incident Report', actual={lines[0] if lines else '(missing)'}"
    ...
    actual_actions=[x for x in lines[lines.index('## Follow-up Actions')+1:] if x.strip()]; assert actual_actions==actions, f'Invalid follow-up actions: expected={actions}, actual={actual_actions}'
