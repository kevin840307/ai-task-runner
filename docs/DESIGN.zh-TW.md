...
`Plan` 必須是只讀操作，將理解結果轉成有界 TODO。每個 TODO 包含 title、description、acceptance_criteria。若模型多次無法輸出合法 JSON，Runner 會依標題、編號、條列、段落與檔案線索做 deterministic fallback，避免整個 24h 流程卡死。
