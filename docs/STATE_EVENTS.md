...
## Resume rules
`--resume` loads compatible state. `--force-new` starts a new run. Script items use independent nested state directories. Agent session ids may be cleared after a task completes; project files and Runner state, not model chat memory, are the durable source of truth.
