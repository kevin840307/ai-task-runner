...
Planning is intentionally domain-neutral. Before returning tasks, the planner runs from the project root and uses read-only read/list/glob/search tools to inspect relevant behavior, dependencies, interfaces, conventions, and verification evidence. Task count follows independently implementable and independently verifiable outcomes, with no fixed range. The quality gate requests at most one corrected plan and checks structural invariants only: duplicate tasks or criteria, uncovered explicit headings/list items, multiple explicit outcomes embedded in one task, and uncovered bracket-tagged validator findings. No planning decision depends on a language-specific word list, technology category, file extension, project name, title phrase, or character-count threshold. If model planning repeatedly fails, deterministic fallback splits only explicit Markdown structure and otherwise keeps one conservative task instead of guessing semantics.
