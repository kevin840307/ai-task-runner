...
Planning is intentionally right-sized. Before returning tasks, the planner runs from the project root and uses read-only read/list/glob/search tools to inspect relevant entry points, dependencies, public interfaces, existing patterns, and tests. It then extracts concrete deliverables and chooses task count from actual complexity: trivial goals can be one task, small tools usually need 2-5 tasks, and broad or multi-file goals may need 6-20 or more independently verifiable tasks. If the model returns too few tasks or planning repeatedly fails, deterministic fallback can split headings, numbered items, bullet items, paragraphs, or dense sentence-level deliverables such as source files, CLI behavior, generated outputs, persistence, tests, templates, configuration, and documentation.
