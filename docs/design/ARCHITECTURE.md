...
`runner.api.run()` retries only transient `RunnerError` service/backend failures; deterministic workflow/state/invariant `RunnerError` failures are fail-closed and are never turned into an unbounded resume loop.

`runner-process.json` is a small detached-UI runtime identity marker owned by the top-level Supervisor. It stores `supervisor_pid`, the current `worker_pid`, `started_at`, `project_root`, and `work_dir`; worker restart updates `worker_pid`, and normal Supervisor exit removes the marker. Before that marker exists, the detached UI may own only a short-lived `.ai-task-runner/ui/launching.json` reservation so one Project cannot be launched twice during startup; the reservation is UI metadata, never Workflow state, and is removed as soon as the live Supervisor marker takes over. The existing `active-process` marker remains Runner-internal child/orphan cleanup state. PID/launch metadata must never drive PASS/FAIL, retry, session, routing, or resume decisions. Runtime control stays deliberately small: a detached UI may create `stop.request` in the work directory; the Supervisor polls for it, terminates the current Worker plus owned child process, removes the request, and exits with code 130. Resume and rerun remain normal CLI launches using `--resume` and `--force-new`; there are no `resume.request` or `rerun.request` files.
...
`workflow/system/*.yaml` contains only `stages` and top-level `flow`. `workflow/registry.py` is intentionally minimal: Stage behavior `type -> class` only. `workflow/loader.py` normalizes Stage instances and validation capability. `workflow/rules.py` reduces durable state; Pipeline owns resume and recovery routing. For top-level `PlanStage`, `workflow/loader.py` expands the built-in `Task -> Review -> Repair(on FAIL) -> Review` per-TODO lifecycle internally, so normal YAML does not repeat it. Explicit `scope: task` remains an advanced static SOP for non-Plan/custom task producers. `PlanStage` stores TODO content only; there is no generated-step queue, `expand`, `foreach`, or separate subflow DSL.
...
The local UI uses adaptive non-overlapping polling: runtime/status/Studio guard requests are serialized, slow down while the browser tab is hidden, and refresh immediately when it becomes visible again. Runtime rendering is signature-based so unchanged state does not repaint the DOM. This keeps the UI lightweight during long-running 24H sessions without changing Runner state or scheduling semantics.
