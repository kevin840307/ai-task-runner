...
- Finalize/Judge/Rewrite: same main work session and compact prompts containing only new instructions/feedback. Planning is logically read-only; Qwen keeps bounded read tools and at least `read_file` available so strict APIs never receive an empty tool set. Fresh/rebuilt planning prompts are self-contained.
