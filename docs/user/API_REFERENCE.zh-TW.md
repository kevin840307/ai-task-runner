...
`workflow_file` 只會 normalization 一次，成為 `RuntimeConfig.workflow`；格式與 User Guide 的線性 Workflow 相同。未指定時，direct request 與每個 YAML List item 都依自己的 validator 設定選擇 `workflow/system/mixed.yaml`、`workflow/system/file.yaml` 或 `workflow/system/ai.yaml`。明確指定的 parent Workflow 會由 YAML child 繼承；item 提供自己的 `workflow_file` 時，`dataclasses.replace()` 直接帶入已 normalization 的 child Workflow，不建立第二條執行路徑。
