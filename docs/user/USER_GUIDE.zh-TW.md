...
可選欄位包含 `validator_prompt`、`ai_validator_prompt`/`ai_validator_prompt_file` 二選一、`ai_validator_count`、`ai_validator_required_passes`、`project_root`。每筆相對 `project_root` 以外層 `--project-root` 為基準；未指定時維持共用 root。所有 Runner-managed state 統一放在外層 `<project-root>/.ai-task-runner`；即使 item 編輯不同的 project root，也只會在該目錄下建立隔離的 `script/<index>`。完成判定與 resume 使用這些 canonical state path。內層 runtime 結束後會恢復外層 script runtime，避免 Plugin/Event/State context 互相污染。
