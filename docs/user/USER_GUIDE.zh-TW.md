...
預設 read-only safety mode 是 `restore`。`--readonly-safety observe` 仍會保留 protected-path enforcement，但 read-only 階段觀察到的一般 project 變更不會被復原，只會記錄為 observed changes。這適合 concurrent project activity，但不會識別是誰做了變更。
