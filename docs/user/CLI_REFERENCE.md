...
| `--readonly-safety` | read-only Stage change handling: `restore` or `observe` | `restore`; `observe` reports ordinary read-only changes without restoring them, while protected paths are still restored |
